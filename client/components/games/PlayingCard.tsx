...
export default function PlayingCard({ rank, suit, faceDown, selected, onClick, size = 'md', className, style }: PlayingCardProps) {
        ...
        return (
            <div className={cn(
                ...
                <div className="relative w-full h-full flex items-center justify-center">
                    <span className={cn('block rotate-45 border-2 border-cyan-100/65', size === 'sm' ? 'h-3 w-2' : 'h-4 w-2.5')} />
                </div>
            </div>
        );
    }
    ...
    return (
        <button
            ...
            style={{ width: dimensions.width, height: dimensions.height, ...style }}
        >
            <span className={cn('text-sm font-black self-start leading-none', size === 'sm' && 'text-[9px]', isRed ? 'text-red-500' : 'text-slate-900')}>
                {rank}
            </span>
            <span className={cn('text-lg leading-none', size === 'sm' && 'text-sm', isRed ? 'text-red-500' : 'text-slate-900')}>
                {SUIT_SYMBOL[suit]}
            </span>
            <span className={cn('text-sm font-black self-end leading-none rotate-180', size === 'sm' && 'text-[9px]', isRed ? 'text-red-500' : 'text-slate-900')}>
                {rank}
            </span>
        </button>
    );
